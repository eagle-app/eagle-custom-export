...
module.exports = class {
    ...
    static createFolder = async (folderPath) => {
        ...
        try {
            const exists = await this.exist(folderPath);
            if (!exists) {
    ...
    static save = async (filePath, data) => {
        const decodeBase64Image = (dataString) => {
            ...
            if (matches.length !== 3) {
                return new Error(i18next.t('error.invalidInputString'));
            }
    ...
    static isEqualExt(ext, formatExt) {
        ...
        if (a === b) {
            throw i18next.t('error.equalExtension');
        }
    }
    ...
    // 生成安全的暫存檔案路徑，避免檔名衝突
    static generateTempFilePath = (tempDir, extension = '.png') => {
        const uuid = crypto.randomUUID();
        const timestamp = Date.now();
        return path.join(tempDir, `${timestamp}-${uuid}${extension}`);
    };
};
